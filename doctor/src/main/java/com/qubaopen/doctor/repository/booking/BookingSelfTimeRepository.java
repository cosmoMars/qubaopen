...
public interface BookingSelfTimeRepository extends MyRepository<BookingSelfTime, Long> {
	...
	@Query("from BookingSelfTime bst where date_format(bst.startTime, '%Y-%m-%d %H:%M') >= date_format(:startTime, '%Y-%m-%d %H:%M') and date_format(bst.endTime, '%Y-%m-%d %H:%M') <= date_format(:endTime, '%Y-%m-%d %H:%M')")
	List<BookingSelfTime> findByTime(@Param("startTime") Date startTime, @Param("endTime") Date endTime);
}
