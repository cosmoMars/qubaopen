...
//            this.context.fillText("游戏结束", this.canvas.width / 2, y+h/5 );
        this.context.fillText("本次得分:"+this.hero.grade, this.canvas.width / 2, y+h/3 );
...
//            var x= getEventPosition(e);
        clearInterval(App.moveTimer);


        if ( x>=0 && x <=200 ) {
            App.moveTimer = setInterval(function(){
                if(App.hero.x>=0){
                    App.hero.x -= 2;
                }
            },1);
            ...
            App.moveTimer = setInterval(function(){
                if(App.hero.x<=450- App.hero.width){
                    App.hero.x += 2;
                }
            },1);
        }
...
function showDialog(data){
    ...
    closable.setting({
        ...
        'message':
        "<p>1.活动时间:1月22日——3月5日,活动范围限上海、北京、天津地区参加。 </p>"+
        "<p>2.优惠券仅限门店购买新年新品大蛋糕时抵扣，不找零，不兑现，不可叠加使用。 </p>" +
        "<p>3.不可与店内其他优惠共同使用。 </p>" +
        "4.本活动最终解释权归面包新语所有。\n" +
